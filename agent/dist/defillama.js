export class DefiLlamaService {
    ...
    async getUsdcYields() {
        try {
            ...
            const usdcPools = data.data
                .filter((pool) => pool.symbol.toLowerCase().includes("usdc") &&
    ...
    async getApyForChains(chains) {
        const pools = await this.getUsdcYields();
        const result = new Map();
        for (const chain of chains) {
            const pool = pools.find((p) => p.chain.toLowerCase() === chain.toLowerCase());
            if (pool) {
    ...
    getFallbackData() {
        ...
        return [
            ...
            { project: "Aave V3", chain: "Arbitrum", apy: 5.1, tvl: 200000000, symbol: "USDC" },

...
const tools = [
        ...
        function: {
            ...
            parameters: {
                ...
                properties: {
                    date: {
                        type: "string",
                        description: "The payroll date and time in ISO format (e.g., '2025-06-15T14:30:00Z') or natural language like '31st January 2025 at 3:00 PM UTC'",
                    },
                },
                required: ["date"],
            },
    ...
    },
    {
        ...
        function: {
            ...
            description: "Get expected yield/APY for USDC deposits from DeFi protocols",
            parameters: {
                type: "object",
                properties: {
                    chain: {
    ...
    },
    {
        ...
        function: {
            ...
            description: "Calculate expected return based on deposit amount. Yield is calculated from now (deposit time) until the payroll date. Payroll date must be set first using set_payroll_date.",
            parameters: {
                type: "object",
                properties: {
                    amount: {
    ...
    },
    {
        ...
        function: {
            ...
            description: "Generate the deposit transaction to create the payroll LP position",
            parameters: {
                type: "object",
                properties: {
                    userAddress: {
...
function toContractRecipients(recipients) {
    if (!recipients)
        return [];
    return recipients
...
async function executeTool(name, args, pendingPayroll) {
    ...
    switch (name) {
        case "set_payroll_date": {
            ...
            if (isNaN(parsedDate.getTime())) {
                ...
                const monthMatch = dateStr.match(/(\d{1,2})(?:st|nd|rd|th)?\s*(january|february|march|april|may|june|july|august|september|october|november|december)\s*(\d{4})?/i);
                const timeMatch = dateStr.match(/(\d{1,2}):(\d{2})(?::(\d{2}))?\s*(am|pm)?\s*(utc|gmt)?/i);
                if (monthMatch) {
                    ...
                    if (timeMatch) {
                        ...
                        if (timeMatch[4]?.toLowerCase() === "pm" && hours < 12)
                            hours += 12;
                        if (timeMatch[4]?.toLowerCase() === "am" && hours === 12)
                            hours = 0;
                    }
        ...
        case "parse_csv_recipients": {
            try {
                let recipients = [];
                const aiRecipients = args.recipients;
                ...
                return {
                    result: JSON.stringify({ error: "Failed to parse recipients: " + error.message }),
                    updatedPayroll: updated,
                };
            }
        }
        case "get_expected_yield": {
            ...
            try {
                ...
                return { result: JSON.stringify({ error: error.message }), updatedPayroll: updated };
            }
        }
        ...
        case "get_deposit_transaction": {
            ...
            try {
                ...
                console.warn("Pre-flight check failed:", err.message);
            }
            ...
            const txData = contractService.generateDepositCalldata(totalAmountBigInt, BigInt(updated.payrollDate), contractRecipients);
            ...
            try {
                ...
                console.warn("Failed to persist payroll:", err.message);
            }
            return {
                result: JSON.stringify({
                    to: txData.to,
        ...
        case "get_user_positions": {
            ...
            try {
                ...
                return {
                    result: JSON.stringify(positions.map((p) => ({
        ...
        case "get_employer_payrolls": {
            const employerWallet = args.employerWallet;
            try {
...
2. Ask for the payroll date and time — the user MUST provide a full date AND time (e.g., "15th June 2025 at 2:30 PM UTC" or "2025-06-15T14:30:00Z"). Do NOT ask how many days — always ask for a specific date and time. ALWAYS suggest dates in the future starting from the CURRENT DATE shown above.
...
router.post("/chat", upload.single("file"), async (req, res) => {
    try {
        ...
        let pendingPayroll = session.pendingPayroll
        ...
        while (assistantMessage.tool_calls && assistantMessage.tool_calls.length > 0) {
            ...
            for (const toolCall of assistantMessage.tool_calls) {
                ...
                const { result, updatedPayroll } = await executeTool(toolCall.function.name, args, pendingPayroll);

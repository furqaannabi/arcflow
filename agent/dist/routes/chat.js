...
const tools = [
        ...
        function: {
            ...
            parameters: {
                ...
                properties: {
                    date: {
                        type: "string",
                        description: "The payroll date in ISO format or natural language like '31st January 2025'",
                    },
                },
                required: ["date"],
            },
    ...
    },
    {
        ...
        function: {
            ...
            description: "Parse a CSV string containing employee wallet addresses and payment amounts",
            parameters: {
                type: "object",
                properties: {
                    csvData: {
                        type: "string",
                        description: "CSV data with columns: address,amount (amount in USDC)",
                    },
                },
                required: ["csvData"],
            },
    ...
    },
    {
        ...
        function: {
            ...
            description: "Calculate expected return based on deposit amount and time until payroll",
            parameters: {
                type: "object",
                properties: {
                    amount: {
    ...
    switch (name) {
        case "set_payroll_date": {
            ...
            if (isNaN(parsedDate.getTime())) {
                ...
                const now = new Date();
                const monthMatch = dateStr.match(/(\d{1,2})(?:st|nd|rd|th)?\s*(january|february|march|april|may|june|july|august|september|october|november|december)/i);
                if (monthMatch) {
                    ...
                    pendingPayroll.payrollDate = Math.floor(new Date(year, month, day).getTime() / 1000);
                }
                else {
        ...
        case "calculate_expected_return": {
            ...
            const days = args.daysUntilPayroll;
            try {
        ...
        case "get_deposit_transaction": {
            ...
            const txData = contractService.generateDepositCalldata(pendingPayroll.totalAmount, BigInt(pendingPayroll.payrollDate), pendingPayroll.recipients);
            return JSON.stringify({
                to: txData.to,
                data: txData.data,

...
const yellowChunking = new YellowChunkingService(process.env.AGENT_PRIVATE_KEY, ALCHEMY_API_KEY);
// Health check
...
app.post("/api/yellow/chunk-batch", async (req, res) => {
    try {
        ...
        const chunked = await yellowChunking.chunkBatch(BigInt(payrollId), targetChainId ? parseInt(targetChainId) : undefined);
        if (chunked) {
